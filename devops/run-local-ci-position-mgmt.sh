#!/bin/bash
# Script to run CI locally for position-mgmt with support for External Validator (for ARM64/Apple Silicon)
...
echo -e "${BLUE}🚀 Running Position Management CI pipeline (Smart Validator Detection)${NC}"
echo ""
...
if [ "$USE_EXTERNAL" = true ]; then
    ...
    if ! pgrep -f solana-test-validator > /dev/null; then
        ...
        cd "$PROJECT_ROOT/contracts/programs/gdx-position-mgmt/position-mgmt"
    ...
    act \
        --workflows contracts/programs/gdx-position-mgmt/.github/workflows/position-mgmt-ci.yml \
        --job lint-and-test \
