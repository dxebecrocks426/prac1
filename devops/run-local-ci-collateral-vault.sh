#!/bin/bash
# Script to run CI locally with support for External Validator (for ARM64/Apple Silicon)
...
echo -e "${BLUE}🚀 Running CI pipeline (Smart Validator Detection)${NC}"
echo ""
...
SUBMODULE_PATH="contracts/programs/gdx-collateral-vault"
...
if [ "$USE_EXTERNAL" = true ]; then
    ...
    act \
        --workflows "$SUBMODULE_PATH/.github/workflows/collateral-vault-ci.yml" \
        --job lint-and-test \
