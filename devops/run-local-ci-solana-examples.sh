#!/bin/bash
# Script to run CI locally for solana-examples with support for External Validator (for ARM64/Apple Silicon)
...
echo -e "${BLUE}🚀 Running Solana Examples CI pipeline (Smart Validator Detection)${NC}"
echo ""
...
if [ "$USE_EXTERNAL" = true ]; then
    ...
    if ! pgrep -f solana-test-validator > /dev/null; then
        ...
        cd "$PROJECT_ROOT/solana-examples"
    ...
    act \
        --workflows solana-examples/.github/workflows/solana-examples-ci.yml \
        --job lint-and-test \
