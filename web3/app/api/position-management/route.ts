...
const PID_FILE = path.join(os.tmpdir(), "position-management.pid");
const LOG_FILE = path.join(os.tmpdir(), "position-management.log");
const PORT = 8081; // Position Management Service port (Settlement Relayer uses 8080)
// Go up from web3 directory to workspace root, then to contracts/programs
const POSITION_SERVICE_DIR = path.join(process.cwd(), "..", "contracts", "programs", "gdx-position-mgmt", "position-mgmt", "position-service");

interface PositionManagementStatus {
  running: boolean;
  pid?: number;
  port: number;
  error?: string;
...
async function getPositionManagementStatus(): Promise<PositionManagementStatus> {
  try {
...
async function startPositionManagement(): Promise<{ pid: number; error?: string }> {
  try {
    // Check if already running
    const status = await getPositionManagementStatus();
    if (status.running) {
      return { pid: status.pid || 0 };
    }

    // Stop any existing processes first
    await stopPositionManagement();
    await new Promise((resolve) => setTimeout(resolve, 500));

    // Check if the position service directory exists
    try {
      await fs.access(POSITION_SERVICE_DIR);
    } catch {
      return {
        pid: 0,
        error: `Position management directory not found: ${POSITION_SERVICE_DIR}`,
      };
    }

    // Check if Cargo.toml exists (Rust project)
    const cargoTomlPath = path.join(POSITION_SERVICE_DIR, "Cargo.toml");
    try {
      await fs.access(cargoTomlPath);
    } catch {
      return {
        pid: 0,
        error: "Position management Cargo.toml not found. Please ensure the Rust project is set up.",
      };
    }
    ...
    // Position service now uses SQLite (matching settlement relayer pattern)
    const dbFileName = "position-service.db";
    const dbPath = path.join(POSITION_SERVICE_DIR, dbFileName);
    const databaseUrl = process.env.DATABASE_URL || `sqlite:${dbPath}`;
    
    // Ensure the database directory exists
    try {
      await fs.mkdir(POSITION_SERVICE_DIR, { recursive: true });
    } catch (error) {
      console.warn(`[Position Management] Could not create directory: ${error}`);
    }
    
    console.log(`[Position Management] Database URL: ${databaseUrl.substring(0, 80)}...`);

    // Check if binary exists, build if needed
    const binaryPath = path.join(
      POSITION_SERVICE_DIR,
      "target",
      "release",
      process.platform === "win32" ? "position-service.exe" : "position-service"
    );

    // Check binary modification time vs source code to determine if rebuild is needed
    // Always rebuild if binary is older than source files (or doesn't exist)
    let useBinary = false;
    try {
      const binaryStats = await fs.stat(binaryPath);
      const apiRsPath = path.join(POSITION_SERVICE_DIR, "src", "api.rs");
      try {
        const sourceStats = await fs.stat(apiRsPath);
        // Use binary only if it's newer than the source file
        useBinary = binaryStats.mtime > sourceStats.mtime;
      } catch {
        // Source file doesn't exist? That's weird, but rebuild anyway
        useBinary = false;
      }
    } catch {
    ...
    try {
      ...
      console.log(`Starting position management with DATABASE_URL: ${databaseUrl.substring(0, 50)}...`);
      
      if (useBinary) {
        // Use pre-built binary (much faster)
        positionProcess = spawn(binaryPath, [], {
          cwd: POSITION_SERVICE_DIR,
          detached: true,
      ...
      return {
        pid: 0,
        error: `Failed to spawn position management process: ${errorMsg}`,
      };
    }

    if (!positionProcess.pid) {
      return {
        pid: 0,
        error: "Failed to start position management: Process PID is undefined",
      };
    }

    const pid = positionProcess.pid;
    ...
    positionProcess.stdout?.pipe(logStream);
    positionProcess.stderr?.pipe(logStream);
    
    // Also log to console for debugging
    positionProcess.stdout?.on("data", (data) => {
      console.log(`[Position Management stdout] ${data.toString()}`);
    });
    positionProcess.stderr?.on("data", (data) => {
      console.error(`[Position Management stderr] ${data.toString()}`);
    });
    ...
    if (!isStillRunning) {
      ...
      return {
        pid: 0,
        error: `Position management process crashed: ${errorDetails}`,
      };
    }

    // Detach process so it continues running after parent exits
    positionProcess.unref();
    ...
    if (!(await checkProcessRunning(pid))) {
      ...
      return {
        pid: 0,
        error: `Position management process failed to start: ${errorDetails}`,
      };
    }
    ...
    return {
      pid: 0,
      error: `Failed to start position management: ${errorMessage}`,
    };
  }
}

async function stopPositionManagement(): Promise<void> {
  try {
    ...
    // Also kill any processes using port 8081 or 8080 (old port) or matching the process name
    try {
      // Kill processes on both old port (8080) and new port (8081) to catch old binaries
      const portsToCheck = [PORT, 8080];
      for (const checkPort of portsToCheck) {
        if (process.platform === "win32") {
          // Find process using port
          const { stdout } = await execAsync(
            `netstat -ano | findstr :${checkPort} || echo ""`
          );
          if (stdout.trim()) {
            ...
            for (const line of lines) {
              ...
              if (processPid && !isNaN(parseInt(processPid))) {
                // Kill any process using these ports (they're position management ports)
          ...
          await execAsync(`lsof -ti:${checkPort} | xargs kill -9 2>/dev/null || true`);
        }
      }
      
      // Also kill by process name
      if (process.platform === "win32") {
        await execAsync(`taskkill /F /IM cargo.exe /FI "WINDOWTITLE eq *position-service*" 2>nul || echo ""`);
        await execAsync(`taskkill /F /IM position-service.exe 2>nul || echo ""`);
      } else {
        // Kill by process name pattern (more specific)
        await execAsync(`pkill -9 -f "position-service" || true`);
      }
    } catch {
    ...
    console.error("Error stopping position management:", error);
  }
}
...
export async function POST(request: NextRequest) {
  try {
    const result = await startPositionManagement();
          ...
          if (logContent.includes("error") || logContent.includes("Error") || logContent.includes("failed")) {
            ...
            if (result.pid && !(await checkProcessRunning(result.pid))) {
              return NextResponse.json(
                {
                  success: false,
                  error: `Position management process crashed. Check logs: ${LOG_FILE}`,
    ...
    if (!isReady && result.pid) {
      ...
      if (!stillRunning) {
        ...
        return NextResponse.json(
          {
            success: false,
            error: "Position management process crashed during startup",
    ...
    return NextResponse.json({
      ...
      ready: isReady,
      message: isReady
        ? "Position management started successfully"
        : "Position management started but health check failed (may still be compiling - check logs)",
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "Position management stopped successfully",
    });
  } catch (error: unknown) {
    const errorMessage = error instanceof Error ? error.message : "Unknown error";
    return NextResponse.json(
