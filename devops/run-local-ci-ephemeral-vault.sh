#!/bin/bash
# Script to run CI locally for ephemeral-vault with support for External Validator (for ARM64/Apple Silicon)
...
echo -e "${BLUE}🚀 Running Ephemeral Vault CI pipeline (Smart Validator Detection)${NC}"
echo ""
...
if [ "$USE_EXTERNAL" = true ]; then
    ...
    if ! pgrep -f solana-test-validator > /dev/null; then
        ...
        cd "$PROJECT_ROOT/contracts/programs/gdx-ephemeral-vault/ephemeral-vault"
    ...
    act \
        --workflows gdx-ephemeral-vault/.github/workflows/ephemeral-vault-ci.yml \
        --job lint-and-test \
