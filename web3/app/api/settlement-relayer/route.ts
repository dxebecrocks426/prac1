...
const PID_FILE = path.join(os.tmpdir(), "settlement-relayer.pid");
const LOG_FILE = path.join(os.tmpdir(), "settlement-relayer.log");
const PORT = 8080;
// Go up from web3 directory to workspace root, then to services
const RELAYER_SERVICE_DIR = path.join(process.cwd(), "..", "services", "gdx-settlement-relayer", "settlement-relayer", "relayer-service");

interface SettlementRelayerStatus {
  running: boolean;
  pid?: number;
  port: number;
  error?: string;
...
async function getSettlementRelayerStatus(): Promise<SettlementRelayerStatus> {
  try {
...
async function startSettlementRelayer(): Promise<{ pid: number; error?: string }> {
  try {
    // Check if already running
    const status = await getSettlementRelayerStatus();
    if (status.running) {
      return { pid: status.pid || 0 };
    }

    // Stop any existing processes first
    await stopSettlementRelayer();
    await new Promise((resolve) => setTimeout(resolve, 500));

    // Check if the relayer service directory exists
    try {
      await fs.access(RELAYER_SERVICE_DIR);
    } catch {
      return {
        pid: 0,
        error: `Settlement relayer directory not found: ${RELAYER_SERVICE_DIR}`,
      };
    }

    // Check if Cargo.toml exists (Rust project)
    const cargoTomlPath = path.join(RELAYER_SERVICE_DIR, "Cargo.toml");
    try {
      await fs.access(cargoTomlPath);
    } catch {
      return {
        pid: 0,
        error: "Settlement relayer Cargo.toml not found. Please ensure the Rust project is set up.",
      };
    }
    ...
    const dbPath = path.resolve(RELAYER_SERVICE_DIR, "relayer.db");
    // Ensure the database directory exists and create empty database file
    // SQLite/sqlx requires the file to exist before connecting
    try {
      const dbDir = path.dirname(dbPath);
      await fs.mkdir(dbDir, { recursive: true });
      // Create empty database file if it doesn't exist (SQLite needs this)
      try {
      ...
      return {
        pid: 0,
        error: `Failed to prepare database file: ${error instanceof Error ? error.message : String(error)}`,
      };
    }
    const databaseUrl = process.env.DATABASE_URL || `sqlite:${dbPath}`;
    const positionMgmtProgramId = process.env.POSITION_MGMT_PROGRAM_ID || "Gdu24TgaP7HAuuBqBx4RKcBgGN29kKUodZFTKVmJNZYn";
    
    console.log(`[Relayer] Database URL: ${databaseUrl.substring(0, 80)}...`);
    ...
    const binaryPath = path.join(
      ...
      "target",
      "release",
      process.platform === "win32" ? "relayer-service.exe" : "relayer-service"
    );
    ...
    try {
      ...
      console.log(`Starting relayer with DATABASE_URL: ${databaseUrl.substring(0, 50)}...`);
      
      if (useBinary) {
        // Use pre-built binary (much faster)
        relayerProcess = spawn(binaryPath, [], {
          cwd: RELAYER_SERVICE_DIR,
          detached: true,
          ...
          env: {
            ...
            POSITION_MGMT_PROGRAM_ID: positionMgmtProgramId,
            RUST_LOG: "relayer_service=debug,info",
          },
        });
      } else {
        // Build first, then run (slower but works if not built)
        // Note: This will take 30+ seconds on first run
        relayerProcess = spawn("cargo", ["run", "--release"], {
          cwd: RELAYER_SERVICE_DIR,
          detached: true,
          stdio: ["ignore", "pipe", "pipe"],
          env: {
      ...
      return {
        pid: 0,
        error: `Failed to spawn settlement relayer process: ${errorMsg}`,
      };
    }

    if (!relayerProcess.pid) {
      return {
        pid: 0,
        error: "Failed to start settlement relayer: Process PID is undefined",
      };
    }

    const pid = relayerProcess.pid;
    ...
    relayerProcess.stdout?.pipe(logStream);
    relayerProcess.stderr?.pipe(logStream);
    
    // Also log to console for debugging
    relayerProcess.stdout?.on("data", (data) => {
      console.log(`[Relayer stdout] ${data.toString()}`);
    });
    relayerProcess.stderr?.on("data", (data) => {
      console.error(`[Relayer stderr] ${data.toString()}`);
    });
    ...
    if (!isStillRunning) {
      ...
      return {
        pid: 0,
        error: `Settlement relayer process crashed: ${errorDetails}`,
      };
    }
    ...
    if (!(await checkProcessRunning(pid))) {
      ...
      return {
        pid: 0,
        error: `Settlement relayer process failed to start: ${errorDetails}`,
      };
    }
    ...
    return {
      pid: 0,
      error: `Failed to start settlement relayer: ${errorMessage}`,
    };
  }
}

async function stopSettlementRelayer(): Promise<void> {
  try {
    ...
    // Also kill any processes using port 8080 or matching the process name
    try {
      if (process.platform === "win32") {
        ...
        await execAsync(`taskkill /F /IM cargo.exe /FI "WINDOWTITLE eq *relayer-service*" 2>nul || echo ""`);
      } else {
        // Kill process using port 8080
        await execAsync(`lsof -ti:${PORT} | xargs kill -9 2>/dev/null || true`);
        // Also kill by process name pattern
        await execAsync(`pkill -9 -f "relayer-service" || true`);
      }
    } catch {
    ...
    console.error("Error stopping settlement relayer:", error);
  }
}
...
export async function POST(request: NextRequest) {
  try {
    const result = await startSettlementRelayer();
          ...
          if (logContent.includes("error") || logContent.includes("Error") || logContent.includes("failed")) {
            ...
            if (result.pid && !(await checkProcessRunning(result.pid))) {
              return NextResponse.json(
                {
                  success: false,
                  error: `Settlement relayer process crashed. Check logs: ${LOG_FILE}`,
    ...
    if (!isReady && result.pid) {
      ...
      if (!stillRunning) {
        ...
        return NextResponse.json(
          {
            success: false,
            error: "Settlement relayer process crashed during startup",
    ...
    return NextResponse.json({
      ...
      ready: isReady,
      message: isReady
        ? "Settlement relayer started successfully"
        : "Settlement relayer started but health check failed (may still be compiling - check logs)",
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "Settlement relayer stopped successfully",
    });
  } catch (error: unknown) {
    const errorMessage = error instanceof Error ? error.message : "Unknown error";
    return NextResponse.json(
