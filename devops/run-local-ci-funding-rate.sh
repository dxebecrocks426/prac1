#!/bin/bash
# Script to run CI locally for funding-rate with support for External Validator (for ARM64/Apple Silicon)
...
echo -e "${BLUE}🚀 Running Funding Rate CI pipeline (Smart Validator Detection)${NC}"
echo ""
...
if [ "$USE_EXTERNAL" = true ]; then
    ...
    if ! pgrep -f solana-test-validator > /dev/null; then
        ...
        cd "$PROJECT_ROOT/contracts/programs/gdx-funding-rate/funding-rate"
    ...
    act \
        --workflows gdx-funding-rate/.github/workflows/funding-rate-ci.yml \
        --job lint-and-test \
