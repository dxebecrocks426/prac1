#!/bin/bash
# Script to run CI locally for liquidation-engine with support for External Validator (for ARM64/Apple Silicon)
...
echo -e "${BLUE}🚀 Running Liquidation Engine CI pipeline (Smart Validator Detection)${NC}"
echo ""
...
if [ "$USE_EXTERNAL" = true ]; then
    ...
    if ! pgrep -f solana-test-validator > /dev/null; then
        ...
        cd "$PROJECT_ROOT/services/gdx-liquidation-engine/liquidation-engine"
    ...
    act \
        --workflows gdx-liquidation-engine/.github/workflows/liquidation-engine-ci.yml \
        --job lint-and-test \
