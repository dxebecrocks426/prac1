#!/bin/bash
# Script to run CI locally for settlement-relayer with support for External Validator (for ARM64/Apple Silicon)
...
echo -e "${BLUE}🚀 Running Settlement Relayer CI pipeline (Smart Validator Detection)${NC}"
echo ""
...
if [ "$USE_EXTERNAL" = true ]; then
    ...
    if ! pgrep -f solana-test-validator > /dev/null; then
        ...
        cd "$PROJECT_ROOT/services/gdx-settlement-relayer/settlement-relayer"
    ...
    act \
        --workflows services/gdx-settlement-relayer/.github/workflows/settlement-relayer-ci.yml \
        --job lint-and-test \
    ...
    # Using --network host to allow solana-test-validator to bind to ports
    # Using --bind to mount the current directory for caching and process persistence
    # Using --container-options for host networking (needed for validator ports)
    # The validator runs as a background process that persists across workflow steps
